...
这是一个面向金融交易员 / 量化交易助理岗位的个人研究项目，用 Python 完成指数或 ETF 行情分析、动量信号构建、简单回测和风险指标输出。

项目定位不是“成熟可实盘策略”，而是展示以下能力：
...
我用这个项目练习把交易想法拆成可验证的规则：先定义信号，再定义仓位和交易成本，最后用回测看收益、回撤、波动和交易次数。这个策略本身很朴素，但它帮助我理解了交易不能只看方向判断，还要看风险收益比、回撤、执行成本和不同市场环境下的失效风险。

我不会把这个项目包装成成熟实盘策略。它更像是一个学习型研究项目，用来证明我具备 Python 数据分析、交易逻辑表达、风险指标计算和复盘总结能力。
